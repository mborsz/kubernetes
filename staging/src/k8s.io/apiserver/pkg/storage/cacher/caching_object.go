...
func (o *cachingObject) GetCachedEvent(id runtime.Identifier, watchType string, magic func() (runtime.Object, error)) (runtime.CacheableObject, error) {
	o.cachedEventsLock.Lock()
	defer o.cachedEventsLock.Unlock()

	// TODO: use (id, watchType) as a key.
	// cachedEvents[json, ADDED]
	// cachedEvents[json, DELETE]
	// cachedEvents[protobuf, ADDED]
	if obj, ok := o.cachedEvents[id]; ok { // id = id, type_
		return obj, nil
	}

	watchEvent, err := magic()
	if err != nil {
		return nil, err
	}

	x, err := newCachingObject(watchEvent)
	if err != nil {
		return nil, err
	}
	o.cachedEvents[id] = x
